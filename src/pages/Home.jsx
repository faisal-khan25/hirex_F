...
// Each link is either a real route (`type: 'route'`, rendered as a React
// Router <Link>) or a same-page section on the Home hero (`type: 'scroll'`,
// rendered as a <button> that smooth-scrolls — never a raw `href="#id"`).
const FOOTER_COLS = [
  { heading: 'Job Seekers', links: [
    { label: 'Browse Jobs',    type: 'route', to: '/jobs' },
    { label: 'Create Account', type: 'route', to: '/register' },
    { label: 'Login',          type: 'route', to: '/login' },
  ] },
  { heading: 'Employers', links: [
    { label: 'Post a Job',      type: 'route', to: '/register' },
    { label: 'Recruiter Login', type: 'route', to: '/login' },
  ] },
  { heading: 'Company', links: [
    { label: 'About Us',    type: 'scroll', id: 'about' },
    { label: 'Features',    type: 'scroll', id: 'features' },
    { label: 'Help Center', type: 'scroll', id: 'help' },
...
export default function Home() {
  ...
  /* Cross-page smooth scroll: if the user clicked "About"/"Features"/"Help"
     from a *different* route (e.g. the Contact page footer), scrollNav's
     goToSection() has already navigated here and stashed the target section
     id. Consume it once, right after mount, and scroll to it. */
  ...
  onClick={() => goToSection(navigate, location.pathname, link.id)}
>
  {link.label}
</button>
                      ) : (
