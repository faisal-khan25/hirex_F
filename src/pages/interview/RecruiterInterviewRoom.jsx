...
function RecruiterInterviewRoom() {
  useSEO({ title: 'Live Interview — Recruiter', description: 'Conduct a live video interview with a candidate on HireX.' });
  ...
  useEffect(() => {
    ...
    liveInterviewApi.joinByToken(sessionToken)
      .then(({ data }) => {
        ...
        // NEW: AI Interview Monitoring — backfill any violations recorded
        // before this recruiter session (re)connected, so a page refresh
        // mid-interview doesn't lose earlier alerts from the live feed.
        if (data.liveSessionId) {
          liveInterviewApi.getViolations(data.liveSessionId)
            .then(({ data: log }) => {
              ...
              })).reverse(); // newest first, matching the live feed's ordering
  ...
  const initSignaling = useCallback(async (mediaStream) => {
    ...
    const unsubViolation = stompService.subscribe(
      `/topic/violation/${sessId}`,
      (msg) => {
    ...
    cleanupFnsRef.current.push(unsubViolation);
