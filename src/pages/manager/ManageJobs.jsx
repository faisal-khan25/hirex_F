...
export default function ManageJobs() {
  ...
  useEffect(() => {
    ...
    getRecruiterApplicantStats(data)
  ...
  return (
    <div className="mj-page">
      <div className="mj-inner">
        ...
        {/* ── Stats strip ─────────────────────────────────────────────
             PERF FIX (CLS): this block used to be wrapped in `{!loading && ...}`,
             meaning it didn't exist in the DOM at all until the fetch resolved —
             when it then popped in, it pushed the job list card straight down,
             which is exactly what Cumulative Layout Shift penalizes. It's now
             always rendered at the same grid size; only the numbers inside swap
             from a skeleton bar to the real value, so the page's height never
             changes when data arrives. */}
        <div className="mj-stats">
          <div className="mj-stat-card">
            <div className="mj-stat-icon">📋</div>
            <div>
              <div className="mj-stat-num">{loading ? <span className="mj-stat-skel" /> : activeCount}</div>
          ...
          <div className="mj-stat-card">
            <div className="mj-stat-icon">📥</div>
            <div>
              <div className="mj-stat-num">{loading ? <span className="mj-stat-skel" /> : appliedCount}</div>
              <div className="mj-stat-label">Applied Applicants</div>
            </div>
          </div>
          {(loading || shortlistedCount > 0) && (
            <div className="mj-stat-card">
              <div className="mj-stat-icon">✅</div>
              <div>
                <div className="mj-stat-num">{loading ? <span className="mj-stat-skel" /> : shortlistedCount}</div>
          ...
          {(loading || hiredCount > 0) && (
            <div className="mj-stat-card">
              <div className="mj-stat-icon">🏆</div>
              <div>
                <div className="mj-stat-num">{loading ? <span className="mj-stat-skel" /> : hiredCount}</div>
          ...
          <div className="mj-stat-card">
            ...
            <div>
              <div className="mj-stat-num">
                {loading ? <span className="mj-stat-skel" /> : (activeCount > 0 ? Math.round(appliedCount / activeCount) : 0)}
              </div>
