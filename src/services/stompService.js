...
class StompService {
  ...
  /**
   * /app/live/violation – NEW: AI Interview Monitoring.
   * Reports a single real-time proctoring violation detected client-side
   * (phone/face/multi-face/noise/tab-switch/camera/mic). `severity` and
   * `metadata` are optional — the backend fills in a sensible default
   * severity per type and clamps it server-side regardless.
   */
  sendViolation(sessionToken, violationType, severity = null, message = '', metadata = null) {
    this.send('/app/live/violation', {
      sessionToken,
      violationType,
      severity,
      message,
