 ...
 */
export async function getRecruiterApplicantStats(knownJobs) {
  // PERF FIX: the caller (ManageJobs) already has the jobs list from its own
  // useFetch('/api/manager/jobs') by the time this runs — re-fetching the
  // same endpoint here was a fully redundant round-trip that also blocked
  // the per-job stats calls below behind it (a 2-hop waterfall before any
  // stats work even started). Accept the already-fetched list instead;
  // only fetch it ourselves if a future caller doesn't have it yet.
  const jobs = knownJobs ?? (await api.get('/api/manager/jobs')).data ?? [];
