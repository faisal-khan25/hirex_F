...
export const liveInterviewApi = {
  ...
  // ── NEW: AI Interview Monitoring (Proctoring) ──────────────────────────

  /**
   * GET /api/live-interview/{id}/violations
   * Recruiter-only. Returns the full timestamped violation log for a
   * session (phone/face/multi-face/noise/tab-switch/camera/mic), plus a
   * per-severity summary count.
   */
  getViolations: (liveSessionId) =>
    api.get(`/api/live-interview/${liveSessionId}/violations`),
};
