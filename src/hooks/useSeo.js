 ...
 *                  // object or array of objects — injected as
 *                  // <script type="application/ld+json"> and removed
 *                  // again on unmount/change so route navigation in
 *                  // this SPA never leaves stale schema behind.
 *   });
 */
export default function useSEO({ title, description, image, url, jsonLd } = {}) {
  useEffect(() => {
    ...
    // Tagged with data-seo-jsonld so it's unambiguous which script this
    // hook owns (the static Organization schema in index.html has no
    // such tag and is left alone).
    let jsonLdEl = null;
    if (jsonLd) {
